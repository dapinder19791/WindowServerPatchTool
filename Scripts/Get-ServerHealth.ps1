...
Param ($computername)
    ...
    try{
               ...
               if($flag)
               {
                ...
                net use \\$computername  $password /USER:$userid $powerShellArgs 2> $null
                Copy-Item  -destination \\$computername\c$ -Recurse  -Path .\scripts\Get-ServerDetails.bat -Force -ErrorAction SilentlyContinue
                Copy-Item -destination \\$computername\c$ -Recurse -Path   .\scripts\Get-ServerDetails.ps1 -Force -ErrorAction SilentlyContinue
                $message="SUCCESS"
               }
              else{
...
function CreatePs
{
  [cmdletbinding()]
   ...
   $batfilename="Get-ServerDetails.bat"
   $Psfilename="Get-ServerDetails.ps1"
...
Function Format-Log {
    ...
    $Updatereport = @()
    #Check for logfile
    If (Test-Path "\\$computername\c$\$($computername)_ServerDetailslog.csv") {
        #Retrieve the logfile from remote server
        
         
        $file = Import-Csv "\\$computername\c$\$($computername)_ServerDetailslog.csv" 
        $Updatereport=$file
    ...
    Else {
       $temp = "" | Select Computer, Title, KB,IsDownloaded,LastRebootTime,LastPatchInstalledTime,Notes,OSVersion,SqlVersion
        ...
        $temp.Title = "ERROR"
        $temp.KB = "File Path Error \\$computername\c$\$($computername)_ServerDetailslog.csv"
        $temp.IsDownloaded = "File Path Error \\$computername\c$\$($computername)_ServerDetailslog.csv"
        $temp.LastRebootTime = "File Path Error \\$computername\c$\$($computername)_ServerDetailslog.csv"
        $temp.LastPatchInstalledTime = "File Path Error \\$computername\c$\$($computername)_ServerDetailslog.csv"
        $temp.Notes = "File Path Error \\$computername\c$\$($computername)_ServerDetailslog.csv"
        $temp.OSVersion  = "File Path Error \\$computername\c$\$($computername)_ServerDetailslog.csv"
        $temp.SqlVersion= "File Path Error \\$computername\c$\$($computername)_ServerDetailslog.csv"
        $Updatereport = $temp      
        }
...
if(Test-Connection -ComputerName $computername -Count 1 -Quiet)
   {
    ...
    If (Test-Path C:\psscripts\psexec.exe) {
        ...
        $returnmessage = Create-VBS -computer $Computername  
       

        if($returnmessage -eq "SUCCESS")
        { 
        C:\psscripts\PsExec.exe -s \\$computername   C:\Get-ServerDetails.bat /quiet /norestart /accepteula -h $powerShellArgs 2> $null 
        $path1 ="\\$computername\c$\Get-ServerDetails.bat"
        $path2 = "\\$computername\c$\Get-ServerDetails.ps1"
